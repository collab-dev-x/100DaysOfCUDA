...
int main() {
    ...
    size_t matrix_A_size = M * M * sizeof(float);
    ...
    std::vector<float> h_A_data(M * M, 1.0f);
    std::vector<float> h_B_data(M * N, 2.0f);
    std::vector<float> h_C_data(M * N, 0.0f);
    ...
    cublasSsymm(handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_UPPER, M, N, &alpha, d_A, M, d_B, M, &beta, d_C, M);
